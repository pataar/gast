...
// writeConfig resets viper and writes content to a temp config.toml, returning its path.
...
func TestLoad_MissingGitLabHost(t *testing.T) {
	cfgFile := writeConfig(t, `token = "glpat-abc"`)
...
func TestLoad_MissingToken(t *testing.T) {
	cfgFile := writeConfig(t, `gitlab_host = "https://gitlab.example.com"`)
...
// TestLoad_EnvVarsOverrideConfigFile verifies that environment variables beat values from the config file.
