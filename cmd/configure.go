...
func runConfigure(cmd *cobra.Command, args []string) error {
	...
	if existing != nil {
		...
		existing = &config.Config{FilterBots: true}
	}
	...
	filterBots := promptYesNo(scanner, "Hide bot activity (bots that @mention you are still shown)?", existing.FilterBots)
	...
	content := buildTOML(host, token, interval, pageSize, filterBots, showFullProject, notifications)
...
func buildTOML(host, token, interval string, pageSize int, filterBots, showFullProject, notifications bool) string {
	var b strings.Builder
	b.WriteString("# gast configuration — generated by `gast configure`\n\n")
	fmt.Fprintf(&b, "filter_bots = %t\n", filterBots)
